...
CANONICAL_CLASS_NAMES = ["Freq", "Mix", "Range", "SRange", "non-SF"]
LABEL_MAP = {
    "Freq": "Freq",
    "Mix": "Mix",
    "Range": "Range",
...
def canonicalize_label(label: Any) -> str:
    if label is None:
        raise ValueError("Label is None and cannot be canonicalized.")
    key = str(label).strip()
    if key in LABEL_MAP:
...
def infer_gt_label_from_path(path: str | Path) -> str:
    return canonicalize_label(Path(path).parent.name)


def build_export_meta(raw_item: Dict[str, Any], dataset_idx: int, sample_id: str, sample_dir: Path, out_T: int = 2) -> Dict[str, Any]:
    target_paths = [str(p) for p in raw_item.get("target_paths", [])]
    if len(target_paths) < out_T:
        raise ValueError(
            f"Sample idx={dataset_idx} target_paths length is {len(target_paths)}, expected at least {out_T}."
        )
    pred_paths = [str((sample_dir / f"pred_{i+1:02d}.png").resolve()) for i in range(out_T)]
    meta = {
        ...
        "gt_label_pred1": infer_gt_label_from_path(target_paths[0]),
        "gt_label_pred2": infer_gt_label_from_path(target_paths[1]),
...
def save_meta_json(sample_dir: Path, meta: Dict[str, Any]) -> None:
    json_dump(meta, sample_dir / "meta.json")
...
def make_relative_symlink(src: Path, dst: Path) -> None:
    ...
    os.symlink(rel_src, str(dst))


def build_cls_eval_symlinks(manifest_rows: Sequence[Dict[str, Any]], cls_eval_dir: Path) -> Dict[str, int]:
    ...
    for row in manifest_rows:
        ...
        sample_id = sanitize_name(str(row["sample_id"]))

        for split_name, gt_key, pred_key in [
            ("pred1", "gt_label_pred1", "pred1_path"),
            ("pred2", "gt_label_pred2", "pred2_path"),
        ]:
            class_name = canonicalize_label(row[gt_key])
            src = Path(str(row[pred_key])).resolve()
            dst = cls_eval_dir / split_name / class_name / f"idx_{dataset_idx:06d}__{sample_id}__{split_name}.png"
...
def export_selected_samples(
    ...
    for idx in tqdm(idxs, desc="Export selected", dynamic_ncols=True):
        ...
        if save_pred_only and pred_frames_dir is not None:
            ...
            meta = build_export_meta(raw_item=raw_item, dataset_idx=idx, sample_id=sample_id, sample_dir=sample_dir, out_T=pred.shape[0])
            save_meta_json(sample_dir, meta)
...
def main() -> None:
    ...
    all_pred_manifest_path = Path(args.all_pred_manifest_path) if args.all_pred_manifest_path else output_dir / "pred_frames_manifest.jsonl"
    cls_eval_dir = Path(args.cls_eval_dir) if args.cls_eval_dir else output_dir / "cls_eval"
    ...
    if should_export_selected:
        ...
        export_t0 = time.time()
        _selected_manifest_rows = export_selected_samples(
            model=model,
            dataset=dataset,

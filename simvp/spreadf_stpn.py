...
class FactorizedGatedSpatiotemporalTransformerBlock(nn.Module):
    def __init__(
        self,
        dim: int,
        heads: int = 8,
...
class FactorizedGatedSpatiotemporalTransformerStack(nn.Module):
    def __init__(
        self,
        dim: int,
        depth: int,
...
class FactorizedGatedSpatiotemporalTransformer(nn.Module):
    def __init__(
        self,
        dim: int,
        depth: int = 2,
        ...
        super().__init__()
        self.temporal_transformer = FactorizedGatedSpatiotemporalTransformerStack(
            dim=dim,
            depth=depth,
            heads=heads,
            ffn_ratio=ffn_ratio,
        ...
        self.spatial_transformer = FactorizedGatedSpatiotemporalTransformerStack(
            dim=dim,
            depth=depth,
            heads=heads,
            ffn_ratio=ffn_ratio,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Factorized gated spatiotemporal modeling: temporal tokens first, then spatial tokens.
        # [B, T, C, H, W] -> [B * H * W, T, C]
...
class FRegionResidualRefinementModule(nn.Module):
    ...
    def __init__(
        ...
        if in_T <= 0 or out_T <= 0:
            raise ValueError(
                f"FRegionResidualRefinementModule expects positive in_T/out_T, got {in_T}, {out_T}."
            )
        if hidden_dim <= 0:
...
class SpreadFSTPN(nn.Module):
    def __init__(
        ...
        if height % 32 != 0 or width % 32 != 0:
            raise ValueError("SpreadFSTPN expects image height and width to be divisible by 32.")
        if depth < 1:
            raise ValueError(f"depth must be >= 1, but got {depth}.")
        if in_T <= 0 or out_T <= 0:
            raise ValueError(f"SpreadFSTPN expects positive in_T/out_T, got {in_T}, {out_T}.")
        ...
        self.translator = FactorizedGatedSpatiotemporalTransformer(
        ...
        if self.use_local_branch:
            ...
            self.local_refiner = FRegionResidualRefinementModule(
                in_T=in_T,
                out_T=out_T,
    ...
    def forward(
        ...
        if self.local_refiner is not None and x_local is not None:
            # Apply the F-region Residual Refinement Module to refine the fixed F-region in image space.
...
SpreadFSpatiotemporalPredictionNetwork = SpreadFSTPN
FGST = FactorizedGatedSpatiotemporalTransformer
FGSTB = FactorizedGatedSpatiotemporalTransformerBlock
FRRM = FRegionResidualRefinementModule
